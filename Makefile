...
ifeq ($(OSTYPE),linux)
	...
	BIN_TARGETS=prism.linux xprism.linux psv.linux
...
MAKE_DIRS = dd jdd odd dv prism mtbdd sparse hybrid parser settings userinterface pepa/compiler simulator jltl2ba jltl2dstar explicit pta param strat psv
...
tests: testslocal
	@if [ -d ~/prism/prism-tests ]; then \
	  cd ~/prism/prism-tests && $(PWD)/etc/scripts/prism-auto -t -m . -p $(PWD)/bin/prism --nailgun --ngprism $(PWD)/bin/ngprism; \
	else \
	  echo "Skipping tests"; \
	fi
...
bin_scripts:
	...
	@PPL_DIR=$(PPL_DIR) ./install.sh silent
