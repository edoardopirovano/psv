...
public class Module extends ASTElement
{
	...
	public Vector<String> getAllSynchs()
	{
		...
		n = getNumCommands();
		//System.out.println("-- getAllSynchs");
		for (i = 0; i < n; i++) {
			/*** ***/
			if(getCommand(i) instanceof SCommand) {
				//System.out.println("SCommand");
				for(j = 0; j < ((SCommand) getCommand(i)).getNumSynchs(); j++) {
					//System.out.println(((SCommand) getCommand(i)).getSynchs().get(j));
					s = ((SCommand) getCommand(i)).getSynchs().get(j);
